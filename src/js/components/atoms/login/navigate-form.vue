...
export default {
    ...
    methods: {
        ...
        createLinkHtml(h, schema, end) {
            return (
                <span>
                    <p onclick={this.handleClick.bind(null, schema.onClick)} class="navigate-form__link">
                        {schema.copy}
                    </p>
                    {end ? null : ' or '}
                </span>
    ...
    render(h) {
        return <div class="navigate-form">{this.createContainerHtml(h, this.$props.schema)}</div>;
    }
};
</script>
